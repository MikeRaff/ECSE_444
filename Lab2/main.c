...
int main(void)
{
  ...
  //int DACValue;
  //int max = 255; // 4095 if 12-bit, 255 if 8-bit

  // signals
  uint32_t triangleWave =0;
  uint32_t sawtoothWave =0;
   ...
   *  Assign each signal to a different DAC output channel, idk if this is that. i dont think it is
   */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  for (uint32_t j = 0; j < 16; j++) {
	  	  if (j < 8)
	  		  triangleWave = j*2 ; // * 512;
	  	  else
	  		  triangleWave = (triangleWave - (j%8)) * 2;
